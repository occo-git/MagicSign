...
public class GrpIntro extends GrpStoryLineBase {
    ...
    @Override
    protected int getMusicId() { return 3002; } // origin_of_life_short
    ...
    public void show() {
        ...
        add_F9S2(52000, 3000); // drop
        add_F9S3(53000); // Привет! Я иммунная клетка Нумми
        add_F9S4(57000); // Я помогу тебе Мне известен сопособ уничтожения вирусных клеток!
        add_F9S5(58000, 3000); // sign cell
        add_F9S7(64000); // Если нарисовать знак правильно, ДНК вирусной клетки разрушится!
        add_F9S8(70000); // sign helper
    ...
    private void add_F8S2(long delay) {
        // Кто защитит Землю?
        showText(txt1, .5f, .38f, R.string.intro_frame8_txt1, delay);
    }
    private void add_F8S3(long delay) {
        // Кто станет этим Героем?
        showText(txt2, .5f, .5f, R.string.intro_frame8_txt2, delay);
    }
    //endregion
    ...
    private void add_F9S3(long delay) {
        // Привет!
    ...
    private void add_F9S5(long delay, final long duration) {
        // sign cell
        addStep(delay, new Runnable() {
            @Override public void run() {
                addAnim(new AnimUtil()
    ...
    private void add_F9S7(long delay) {
        ...
        showMessage(R.string.intro_frame10_msg1, true, MessageInfo.MSG_TEMPER_UP, delay, R.raw.energy_pick17);
    }
