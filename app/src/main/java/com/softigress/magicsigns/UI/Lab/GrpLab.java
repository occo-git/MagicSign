...
public class GrpLab extends GrpUserInfos {
    ...
    private static final float xEquals =       .10f, yEquals =         .40f; // .35f;
    private static final float xFinalComment = .50f, yFinalComment =   .70f;
    private static final float xPotionFinal =  .50f, yPotionFinal =    .33f, fdPotionFinal =  .320f;
    ...
    private final MessageText txtFinalComment;
    ...
    public GrpLab() {
        ...
        backPaint = PaintUtils.getPaintWhite(20);
        //frameRate = new DrawingFrameRate("lab", .04f, .8f);
        ...
        //region Comment
        txtFinalComment = new MessageText(DrawingHAlign.CENTER);
        txtFinalComment.setVerticalAlign(DrawingVAlign.BOTTOM);
        txtFinalComment.setFontSize(TextUtils.lab_comment);
        txtFinalComment.isPaintRect = true;
        txtFinalComment.isIdea = true;
        txtFinalComment.setTextBack(4f, 32, 0, 0, 0);
        txtFinalComment.setText(R.string.lab_final_comment);
        txtFinalComment.setPoint(xFinalComment, yFinalComment);
        txtFinalComment.hide();
        addDrawing(txtFinalComment);
        //endregion
    ...
    //region select
    private void selectItem(LabItem labItem) {
        for (LabItem i : labItems)
    ...
    private void selectIndex(LabIndex labIndex) {
        ...
        //    Utils.playSound(R.raw.glass06); // selectedItem.isEnabled() ? R.raw.glass08 : R.raw.glass06
    ...
    @Override
    protected int getMusicId() { return 3004; } // the_potion
    ...
    private boolean isShowHideItems = false;
    private void showhideLabItems(boolean isShowHide) {
        isShowHideItems = isShowHide;
        if (isShowHide) {
            for (LabIndex i: labIndices)
                i.show();
            for (LabItem i : labItems)
    ...
    private void animatePotionFinal() {
        if (!isOnActionFinal && potionFinal.isVisible()) {
            ...
            Utils.playSound(R.raw.glass_bulk05, animDurationFinal / 3);
    ...
    protected void refreshControls() {
        ...
        if (userId == null) {
            ...
            if (userRowInfo != null && userRowInfo.userScoreInfo != null) {
                hideDropWait(100);
                hideSignInComment();
                showhideVideos(!userRowInfo.userScoreInfo.allPotions()); // кнопка диалога Эликсир за видео, если не все еще potion получены
    ...
    private float fxOffset = .2f;
    private float x0, y0, x1, y1, r4;
    @Override
    public synchronized void calc() {
        super.calc();

        //if (isShowHideItems) {
            x0 = (fxOffset - LabIndex.fdIndex / MetrixUtils.screen_K) * MetrixUtils.screen_metrix_width;
            y0 = (yLabIndex1 - LabIndex.fdIndex) * MetrixUtils.screen_metrix_height;
            x1 = (1 - fxOffset + LabIndex.fdIndex / MetrixUtils.screen_K) * MetrixUtils.screen_metrix_width;
            y1 = (yLabIndex2 + LabIndex.fdIndex) * MetrixUtils.screen_metrix_height;
            r4 = LabIndex.fdIndex * MetrixUtils.screen_metrix_height;
        //}
    }
    ...
    protected void drawItems(Canvas c) {
        ...
        if (isShowHideItems)
            // обводка кнопок
            c.drawRoundRect(new RectF(x0, y0, x1, y1), r4, r4, backPaint);
