...
public class GrpMainMenu extends GrpUserInfos {
    ...
    */
    ...
    private static final float xCrown =        .50f, yCrown =      .47f, fdCrown =     .300f;
    private static final float xStart =        .50f, yStart =      .28f;
    private static final float xComment =      .50f, yComment =    .62f;
    private static final float xDrop =         .17f, yDrop =       .60f, fdDrop =      .133f;
    private static final float xScore =        .50f, yScore =      .59f, fdStar =      .030f;
    ...
    public GrpMainMenu() {
        ...
        btnMenu.clickSoundId = R.raw.ui_item_click14;
        ...
        dlgExit.setListener(new IGrpDialogResultListener() {
            @Override
            public void handleOnResult(int result) {
                if (result ==  GrpExitDialog.DIALOG_RESULT_RATE)
                    showDialog(dlgRate);
                else
                    hideDialog();
            }
        ...
        btnRating.clickSoundId = R.raw.ui_item_click14;
        ...
        */
        ...
        btnLab.name = "btn_lab";
        btnLab.clickSoundId = R.raw.ui_item_click14;
        //btnLab.clickSoundId = R.raw.glass08;
        ...
        txtComment.setVerticalAlign(DrawingVAlign.CENTER);
    ...
    //region music
    @Override
    protected int getMusicId() { return 3001; } // origin_of_life
    ...
    private void refreshVideoControls() {
        if (CurrentSettings.isAdvVideo && CurrentSettings.isShowAdvVideo) {
            showVideo.show();
            showVideoOk.show();
        } else {
            showVideo.hide();
    ...
    private void setScore(Integer score, Integer waves, Integer games, final Integer index) {
        ...
        //region Waves
        if (waves > -1) {
            txtComment.setPoint(xComment, yComment - 2f * fdWave);
    ...
    private void animateControls() {
        if (!isOnAction) {
            ...
            if (btnStart.isVisible()) {
                ...
                new AnimUtil()
                        .add(startHalo, "scale", 1f, 3f)
                        .add(startHalo, "alpha", 24, 48, 0)
                        .start(4 * animDuration, dd / 2);
            }
            if (orden.isVisible()) {
                new AnimUtil()
            ...
            if (grpCrown != null) {
                ...
                new AnimUtil(grpCrown, "fy", ffy, ffy + .005f, ffy - .01f, ffy)
                        .startSimple(animDuration, 3 * dd);
            }
    ...
    public synchronized void calc() {
        ...
        x0 = (xLab - fdRating / MetrixUtils.screen_K) * MetrixUtils.screen_metrix_width;
        y0 = (yRating - fdRating) * MetrixUtils.screen_metrix_height;
        x1 = (xRating + fdRating / MetrixUtils.screen_K) * MetrixUtils.screen_metrix_width;
        y1 = (yRating + fdRating) * MetrixUtils.screen_metrix_height;
        r4 = (x1 - x0) / 2f;
    }
