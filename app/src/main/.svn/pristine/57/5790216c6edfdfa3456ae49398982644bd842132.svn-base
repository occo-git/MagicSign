...
public class GrpDialog extends GrpWindow {
    ...
    private boolean isOnShowDialog = false;
    private boolean isOnHideDialog = false;
    protected boolean isUserCanHide = true;
    ...
    protected void setOkEnabled(boolean isEnabled) {
        if (btnOk != null) {
            if (isEnabled)
                btnOk.enable();
            else
    ...
    // пользователь может закрыть диалог
    public boolean isUserCanHide() { return isUserCanHide; }
    // диалог в процессе открытия или зактрытия
    public boolean isOnShowHide() { return isOnShowDialog || isOnHideDialog; }
