...
public class GrpGame extends ActivityGroupBase {
    ...
    private final static float xOptions =   .05f, yOptions =    .088f, fdOptions = .050f;
    ...
    public GrpGame() {
        ...
        drop.addMessageTexts(
                ...
                new int[] {
                    ...
                    0, 1, R.string.message_game_drop_punched_02,
        ...
        // отключаем прорисовку диалогов в drawFrame() рисуем их поверх всего методом drawDialogs
        isDrawDialogs = false;
        ...
        //region Pause button
        btnPause = new BtnBase(xPause, -.35f, fdPause, R.string.bmp_btn_menu);
        btnPause.name = "btn_pause";
        btnPause.clickSoundId = R.raw.ui_item_click14;
        ...
        btnOptions = new BtnBase(xOptions, -.2f, fdOptions, R.string.bmp_btn_options);
        btnOptions.name = "btn_options";
        btnOptions.clickSoundId = R.raw.ui_item_click14;
        btnOptions.setAlpha(128);
        btnOptions.setListener(new IClickListener() { @Override public void handleOnClick(ITouchable e) { onPauseGame(dlgOptions); } });
        addDrawingTouchable(btnOptions);
        //endregion
    ...
    private void onFinish(boolean isWin) {
        if (!isOnExit && !isFinished) {
            ...
            if (Utils.dataBaseManager.currentUserScoreInfo == null && score > 0)
                ...
                TaskUtils.postDelayed(2000, new Runnable() { @Override public void run() { showResult(); } });
            }
        }
    }
            ...
            Utils.authManager.getUserPhoto(new IAsyncLoadFile() {
                ...
                public void onFinish(FileInfo fileInfo) {
                    if (fileInfo.file != null)
                        setUserPhoto(BitmapFactory.decodeFile(fileInfo.file.getAbsolutePath()));
                }
        ...
        if (showhide) {
            if (!isPauseVisible) {
                if (!isOnPauseAction) {
                    ...
                    isPauseVisible = true;
                    if (buttonsAnimSet != null && buttonsAnimSet.isStarted())
                        buttonsAnimSet.cancel();
                    // show
                    buttonsAnimSet = new AnimUtil()
                            .add(btnPause, "fy", -.35f, yPause + fdPause / 3f, yPause)
                            .add(btnOptions, "fy", -.2f, yOptions + fdOptions / 3f, yOptions)
                            .startAD(duration);
                    //Utils.playSound(R.raw.ui_dialog_show02);
                }
            }
            ...
            if (isPauseVisible) {
                if (!isOnPauseAction) {
                    ...
                    // hide
                    buttonsAnimSet = new AnimUtil()
                            .add(btnPause, "fy", btnPause.getFy(), -.35f)
                            .add(btnOptions, "fy", btnOptions.getFy(), -.2f)
                            .startAD(duration);
                    //Utils.playSound(R.raw.ui_dialog_hide02);
                }
            }
